...
export const getPVC = async (
  ...
  let kubeResponse;
  try {
    kubeResponse = await fastify.kube.coreV1Api.readNamespacedPersistentVolumeClaim(pvcName, fastify.kube.namespace);
...
export const createPVC = async (
  ...
  const kubeResponse = await fastify.kube.coreV1Api.createNamespacedPersistentVolumeClaim(fastify.kube.namespace, pvcData);
  return kubeResponse.body as PersistentVolumeClaimKind;
};
